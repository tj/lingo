
/*!
 * Lingo - Language
 * Copyright(c) 2010 TJ Holowaychuk <tj@vision-media.ca>
 * MIT Licensed
 */

/**
 * Module dependencies.
 */

var lingo = require('./lingo');

/**
 * Initialize a new `Language` with the given `code`.
 *
 * @param {String} code
 * @api public
 */

var Language = module.exports = function Language(code) {
  this.code = code;
  this.pluralCount = 2;
  this.pluralFormula = 'n == 1 ? 0 : 1';
  this.translations = {};
  this.rules = {
      plural: []
    , singular: []
    , uncountable: {}
    , irregular: { plural: {}, singular: {}}
  };
  lingo[code] = this;
};

/**
 * Translate the given `str` with optional `params`.
 *
 * @param {String} str
 * @param {Object} params
 * @return {String}
 * @api public
 */

Language.prototype.translate = function(str, params){
  str = this.translations[str] || str;
  if (params) {
    str = str.replace(/\{([^}]+)\}/g, function(_, key){
      return params[key];
    });
  }
  return str;
};

Language.prototype.compilePlural = function(str) {
  str = (this.translations[str] || str).replace("'", "\\'");

  // Build the individual plural forms.
  var forms = [];
  var pluralVariable;
  for (var i = 0; i < this.pluralCount; i++) {
    forms[i] = str.replace(/\{([^:}]+)(?::([^}]+))?\}/g, function(_, key, plurals) {
      key = key.replace("\\'", "'");
      if (plurals) {
        pluralVariable = key;
        var plural = plurals.split('|')[i];
        return plural.replace('@', "' + n + '");
      }
      return "' + (" + key + ") + '";
    });
  }
  
  return new Function('params',
    'with (params) {' +
    'var n = (' + pluralVariable + ');' +
    'var _pluralIndex = (' + this.pluralFormula + ');' +
      forms.map(function(str, i) {
        return 'if (_pluralIndex === ' + i + ") return '" + str + "';";
      }).join('') + 
    '}'
  );
};

/**
 * Choose a plural form based on the parameters.
 *
 * @param {String} str
 * @param {Object} params
 * @return {String}
 * @api public
 */

Language.prototype.translatePlural = function(str, params) {
  var fn = this.translations[str] || str;
  
  if (typeof fn !== 'function') {
    fn = this.translations[str] = this.compilePlural(str);
  }
  
  return fn(params);
};
